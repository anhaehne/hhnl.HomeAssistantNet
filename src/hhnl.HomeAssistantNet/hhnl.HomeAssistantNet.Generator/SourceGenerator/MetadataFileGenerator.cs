...
namespace hhnl.HomeAssistantNet.Generator.SourceGenerator
{
    public class MetadataFileGenerator
    {
            ...
            IReadOnlyCollection<string> entitiesFullNames)
        {
            ...
            var automationInfo = string.Join(Environment.NewLine , verifiedAutomationMethods.Select(automationMethod =>
            {
                ...
                var arguments = string.Join(",", method.Parameters.Select(p => $"s.GetRequiredService<{p.Type.GetFullName(entitiesFullNames)}>()"));
            ...
            {{
                ...
                {nameof(AutomationInfo.RunAutomation)} = async s => 
                {{
            ...
            var registerEntities = string.Join(Environment.NewLine,
                entitiesFullNames.Select(t => $"            serviceCollection.AddSingleton<{EntityClassSourceFileGenerator.EntityNamespace}.{t}>();"));
            ...
            var entityTypes = string.Join(Environment.NewLine, entitiesFullNames.Select(t => $"            typeof({EntityClassSourceFileGenerator.EntityNamespace}.{t}),"));
